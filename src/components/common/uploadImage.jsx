...
export default function ImageUploadToAWSComponent() {
  ...
  const handlfileChange = async (e) => {
    if (e.target.files[0]) {
      if (property?._id) {
        var date = Date.now();
        const propertyPhoto = new File(
          [e.target.files[0]],
          `property_${property?._id}_${date}.${e.target.files[0].name.split(".").pop()}`
        );
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {property.imageURLs.map((image) => (
                        <tr key={image.filename}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <a href={image.url} className="text-sm text-indigo-700 truncate w-2xl italic">
                              Link
                            </a>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
